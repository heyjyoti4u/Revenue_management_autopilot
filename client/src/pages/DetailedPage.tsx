...
export default function DetailedPage({ route }: { route: RouteKey }) {
  ...
  return <div className={`signalstay-site detailed-page detailed-page--${data.theme}`}><SiteHeader /><main>
    <Reveal className="page-reveal page-reveal--hero"><section className="detailed-hero" style={{ backgroundImage: `linear-gradient(90deg, rgba(8,13,34,.98) 15%, rgba(8,13,34,.87) 48%, rgba(8,13,34,.54) 100%), url(${image})` }}><div className="detailed-hero-grid" /><div className="content-frame detailed-hero-inner"><div className="detailed-hero-copy"><p className="eyebrow eyebrow--violet"><span className="eyebrow-pulse" />{data.eyebrow}</p><h1>{data.title}<br /><em>{data.emphasis}</em></h1><p>{data.intro}</p><div className="hero-actions"><Link className="button button--violet" href={data.primaryHref}>{data.primary} <ArrowUpRight size={16} /></Link><Link className="button button--ghost-light" href={data.secondaryHref}>{data.secondary} <ArrowRight size={16} /></Link></div><span className="detailed-hero-foot"><i />SignalStay / {data.theme} / daily operating view</span></div><PageArtifact route={route} /></div></section></Reveal>
    ...
    <Reveal className="page-reveal page-reveal--cta"><section className="detail-cta" style={{ backgroundImage: `linear-gradient(90deg, rgba(8,13,34,.96), rgba(8,13,34,.68)), url(${closingImage})` }}><div className="content-frame"><p className="eyebrow eyebrow--violet">A better read changes the next move</p><h2>{route === "book" ? "Bring us the signal." : "See what is moving"}<br /><em>{route === "book" ? "We will map it together." : "before the calendar."}</em></h2><Link className="button button--violet" href="/book-a-call-with-our-experts">Book a signal review <ArrowUpRight size={16} /></Link></div></section></Reveal>
  </main><SiteFooter /></div>;
}
